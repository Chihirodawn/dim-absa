...
class LogSigmaRegressor(nn.Module):
    def __init__(
        self, hidden_size: int, dropout: float = 0.1, pooling: str = "cls"
    ) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if self.pooling == "mean":
            ...
            counts = mask.sum(dim=1).clamp(
                min=torch.tensor(1.0, dtype=hidden.dtype, device=hidden.device)
            )
...
def predict(model, regressor, loader, device):
    ...
    for batch in loader:
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            scores, _ = regressor(hidden, inputs["attention_mask"])
...
def main() -> None:
    ...
    regressor = LogSigmaRegressor(model.config.hidden_size, pooling=args.pooling)
    ...
    for epoch in range(epochs):
        ...
        for batch_index, batch in enumerate(train_loader, start=1):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                hidden = model(**inputs, return_dict=True).last_hidden_state
                scores, opinion_logits = regressor(hidden, inputs["attention_mask"])
