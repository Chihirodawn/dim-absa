...
### TASK-036：发布 2026-09 Task 2/3 中英文实验总结与模型能力对照

- 状态：已完成（2026-09-21）。
- 范围：只整理已存在的实验记录，没有重新训练、评测或产生新指标。
- README **末尾**已加入 Qwen3-8B seed2971 四个中英文 Test 结果、9月实验过程、失败方法、
  RAFT 尚未运行的边界，以及“约 0.48”为跨划分局部增益估算而非正式结果的更正。
- 同一 English Restaurant Task 3 对照：当前 Qwen3-8B 单次 greedy `0.5781`，旧 Qwen
  三视角混合系统 `0.5735`，Kimi 原始输出 `0.5762`，Kimi + RoBERTa `0.5858`。
- 能力结论：Qwen 负责稳定生成和域内 Category；通用大模型只适合高置信英文 A–O 审查；
  VA 继续使用专门回归头。下一步先做成对候选排序器，再决定是否进行 RAFT/RL。
- 验收：Markdown 结构与链接检查、`git diff --check`，并检查仓库中无新增数据、权重、
  预测、日志或密钥。
